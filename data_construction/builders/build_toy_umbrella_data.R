...
script_path <- if (length(file_arg)) {
  ...
  normalizePath(
    "data_construction/builders/build_toy_umbrella_data.R"
  )
}
...
clean <- raw
clean$umbrellas_per_puddle <- round(
  clean$umbrellas / pmax(clean$puddles, 1),
  2
)
clean$cocoa_per_notebook <- round(
  clean$cocoa_cups / pmax(clean$notebooks, 1),
  2
)
clean$quality_flag <- ifelse(
  clean$puddles > 0 & clean$umbrellas > 0,
  "usable",
  "check"
)
...
write.csv(
  clean,
  file.path(clean_dir, "toy_umbrella.csv"),
  row.names = FALSE
)
...
accounting <- data.frame(
  ...
  reason = c(
    "rows in raw source file",
    "usable toy rows",
    "none in this toy example"
  )
)
write.csv(
  accounting,
  file.path(clean_dir, "toy_umbrella_accounting.csv"),
  row.names = FALSE
)
